...
var UrlSource = class extends Source {
  ...
  constructor(url2, options = {}) {
    ...
    this._url = url2 instanceof URL ? url2 : new URL(url2, typeof location !== "undefined" ? location.href : void 0);
    this._options = options;
      ...
      while (currentPos <= startPos - MIN_BOX_HEADER_SIZE) {
        if (currentFragment?.nextFragment) {
          ...
          if (boxInfo.name === "moof") {
            const index3 = binarySearchExact(this.fragments, boxStartPos, (x) => x.moofOffset);
            let fragment2;
            if (index3 === -1) {
              fragment2 = await this.readFragment(boxStartPos);
            } else {
...
var Conversion = class _Conversion {
  ...
  constructor(options) {
    ...
    if (!(options.input instanceof Input) && !(Array.isArray(options.input) && options.input.length > 0 && options.input.every((input) => {
  ...
  async _init() {
    ...
    const inputAndOutputFormatMatch = this.inputs.length == 1 && (await this.inputs[0].getFormat()).mimeType === this.output.format.mimeType;
